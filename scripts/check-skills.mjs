...
for (const name of skills) {
  ...
  if (fm.name !== name)
    fail(`${name}/SKILL.md frontmatter name mismatch: '${fm.name}'`);
  if (!fm.description) fail(`${name}/SKILL.md missing description`);
  if (fm.version !== pkgVersion)
    fail(
      `${name}/SKILL.md version '${fm.version}' != package.json '${pkgVersion}'`,
    );
}
...
for (const rel of [
  ...
  const versions = rel.endsWith("marketplace.json")
    ? json.plugins.map((plugin) => plugin.version)
    : [json.version];
  for (const version of versions) {
    if (version !== pkgVersion)
      fail(`${rel} version '${version}' != package.json '${pkgVersion}'`);
  }
}

const waText = fs.readFileSync(
  path.join(skillsDir, "working-agreements", "SKILL.md"),
  "utf8",
);

const indexed = new Set(
  [...waText.matchAll(/^\s*- `([a-z0-9-]+)` —/gm)].map((match) => match[1]),
);
...
if (missing.length || extra.length)
  fail(
    `contract stack index drift: missing=${JSON.stringify(missing)} extra=${JSON.stringify(extra)}`,
  );

const versionMatch = waText.match(/\*\*v([0-9][0-9a-z.-]*)\*\*/);
if (!versionMatch || versionMatch[1] !== pkgVersion)
  fail(
    `contract version section mismatch: '${versionMatch ? versionMatch[1] : "none"}' != ${pkgVersion}`,
  );
  ...
  for (const filePath of walk(path.join(root, dir))) {
    ...
    for (const name of staleNames) {
      if (content.includes(name))
        fail(
          `stale reference to '${name}' in ${path.relative(root, filePath)}`,
        );
    }
  }
}
...
console.log(
  `PASS ${skills.length} skills, versions @ ${pkgVersion}, index and README in sync`,
);
